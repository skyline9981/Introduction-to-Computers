...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH,
        ...
        self.start = False
        # make the "YOU LOSE!" picture.
        self.lose = GImage("lose.jpg")
        # make the "YOU WIN!" picture.
        self.win = GImage("win.jpg")
        # make the button which makes you can end the game right now.
        self.end = GLabel("End Game.", 0, 15)
        self.window.add(self.end)
        # make a special label to scare someone.
        self.scare = GLabel("WOW", self.window.width // 2, 15)
        self.scareimg = GImage(random.choice(["1.jpg", "2.jpg", "3.jpg", "4.jpg"]))
        self.window.add(self.scare)
    ...
    # you can click your mouse to start or end the game.
    def play(self, event):
        obj = self.window.get_object_at(event.x, event.y)
        if obj is not self.end:
            self.start = True
        else:
            self.window.close()
